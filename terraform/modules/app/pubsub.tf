# Pub/Sub: ユーザー退会イベント
# user-service が "user-deleted" トピックにパブリッシュし、
# task-service が該当ユーザーのタスクを削除する
...
resource "google_pubsub_topic_iam_member" "user_publisher" {
  project = var.project_id
...
resource "google_pubsub_subscription_iam_member" "task_subscriber" {
  project      = var.project_id
